...
In an attempt to consider this factor, we plot $D_A$ as a function of $\text{OD}\cos\theta r_o^\alpha$ in Fig.~\ref{fig:collision-angle}, where $\alpha=0,1,2$.
...
\begin{align}
   ...
   y_n&=y_{n-1}+u_{y_{n-1}}\dd t,\\
   z_n&=z_{n-1}+(u_{z_{n-1}}-v_s)\dd t,\\
   u^i_{n}&=u^i_{n-1}-\frac{1}{\tau}u^i_{n-1}dt+v\sqrt{\frac{2\Delta t}{\tau}}N^i_{n}
\end{align}
Where $N^{i}_n$ is a random number with zero mean and variance one.
\subsection{One dimensional model}
...
\begin{figure}[h!]
        \centering
        \includegraphics[width=\columnwidth]{figures/ro_t1fit_t2fit.pdf}
        \caption{}
        \label{fig:MSD_different_ro}
\end{figure}
 \begin{figure}[h!]
