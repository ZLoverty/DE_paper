...
\textcolor{blue}{In this section, we report the results from numerical simulation of droplet motions. The idea is to align some simulation results with experimental results, e.g. confinement effect. The current simulation is good on its own, but lacks some alignment with experiment, for example the parameters. If there are new results from simulation, we can get together to discuss what to include in this section.}

\section{Discussions}

\textcolor{blue}{
  In this section, we discuss the results. For the results we understand, we can provide more insight, e.g. by giving more detailed description and analysis of a model. For the results we do not understand yet, we discuss in the context of literature, and pose specific open questions for future research to follow up.
}
